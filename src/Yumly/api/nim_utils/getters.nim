...
func getTuple*(val: Value): seq[Value] =
  if val.kind != vkTuple:
    expectedTypeError("tuple", $val.kind)
  return val.elements

func getTuple*(val: Value, default: seq[Value]): seq[Value] =
  if val.kind != vkTuple: return default
  return val.elements