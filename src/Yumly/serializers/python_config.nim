...
proc valueToPy(value: Value, pyBuiltins: PyObject): PyObject =
  ...
  of vkTuple:
    ...
    for it in value.elements:
      discard pyList.append(valueToPy(it, pyBuiltins))
    result = pyBuiltins.`tuple`(pyList)
...
proc parseValue(value: PyObject, pyTypes: PyTypes, pyBuiltins: PyObject): Value =
  ...
  let isPyTuple = pyBuiltins.callMethod("isinstance", value, pyTypes.`tuple`).to(bool)
  if pyBuiltins.callMethod("isinstance", value, pyTypes.list).to(bool) or isPyTuple:
    var elems: seq[Value] = @[]
    for item in value:
