...
func valueToken(cursor: Cursor, kind: TokenKind, line, col: int,
    value: string): Token =
  case kind
  of tkString, tkIdent, tkLiteral:
    Token(kind: kind, line: line, col: col, endLine: cursor.line,
...
proc tokenize*(stream: Stream, bufferSize: int = 4096): proc(): Token {.closure.} =
  ...
  return proc(): Token {.closure.} =
    while true:
      ...
      of '[': discard cursor.advanceChar(); return cursor.token(tkLBracket,
          line, col)
      of ']': discard cursor.advanceChar(); return cursor.token(tkRBracket,
          line, col)
      of '=': discard cursor.advanceChar(); return cursor.token(tkEquals, line, col)
      of ';': discard cursor.advanceChar(); return cursor.token(tkDeclaration,
          line, col)
      ...
      of '?': discard cursor.advanceChar(); return cursor.token(tkInterrogation,
          line, col)
...
template withTokenPuller*(stream: Stream, pullToken: untyped, body: untyped) =
  ...
  ##
  ## The template owns ``stream`` after entry. Pass a freshly opened stream
  ## unless the caller no longer needs it.
  block:
    let input = stream
    try:
